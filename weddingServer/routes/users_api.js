...
router.post('/doLogin', function (req, res, next) {
  ...
  MongoClient.connect(dbURL, (err, client) => {
    ...
    db.collection('users').find({'password':req.body.password,'username':req.body.username}).toArray(function (err, result) {
      ...
      console.log(result);
      if(result.length>0){ 
        res.json({succ:true,msg:'login successfully',username:result[0].username,f_name:result[0].f_name,l_name:result[0].l_name,key:result[0].key,gender:result[0].gender,email:result[0].email,photo:result[0].photo,phone:result[0].phone,category:result[0].category,description:result[0].description,experience:result[0].experience,dob:result[0].dob,token:encryptionString});
      }else{
